...
#fixes...
logoutUI2 <- function(id, label = "Log out", icon = NULL, class = "btn-danger", style = "color: white;") {
  ns <- shiny::NS(id)
...
db_user <- dbConnect(SQLite(), "db_user_file.sqlite")
dbListTables(db_user)
user_base <- tibble(dbGetQuery(db_user, "SELECT * FROM user_base"))
# sessions <- dbGetQuery(db_user, "SELECT * FROM sessionids")
...
add_sessionid_to_db <- function(user, sessionid, conn = db_user) {
  tibble(user = user, sessionid = sessionid, login_time = as.character(now())) %>%
    dbWriteTable(conn, "sessionids", ., append = TRUE)
}
get_sessionids_from_db <- function(conn = db_user, expiry = cookie_expiry) {
  ...
  mainPanel(
    ...
    loginUI(id = "login", cookie_expiry = cookie_expiry,
            additional_ui = {
              tags$div(
                tags$a(
                  HTML(paste('<a href=?page=signup>Sign Up</a>'))
                )
              )
            }),
    width = 12
...
server <- function(input, output, session) {
    ...
    renderUI({
      page_navbar(
          ...
          if(credentials()$user_auth){
            nav_menu(
              title = paste(user_data()$user),
              icon = icon("gear"),
              nav_item(
                tags$a(
                  div(logoutUI2(id = "logout", class = "primary")),
                  height = 40,
                  title = "",
                )
              )
            ...
            nav_item(
              tags$a(
                strong("Log In"),
                height = 40,
                href = paste(quavaprot_host, "?page=login", sep=""),
                title = ""
              )
            )
          },
  ...
  observeEvent(credentials()$user_auth, {
    if((query$page == "login") & (credentials()$user_auth)){
      session$sendCustomMessage(type = 'redirect', paste(quavaprot_host, "?page=home&data_set=GDC", sep = ""))
    }
  })
  ...
  observeEvent(input$submit_new_user, {
    if((input$new_user == "")||(input$new_password1 == "")||(input$new_email == "")||(input$new_password2 == "")){
      output$signup_errors <- renderText({
      ...
      print(paste(username, password, email))
      if(nchar(username) < 6){
        output$signup_errors <- renderText({
          'Error, username must be at least 6 characters'
        })
      }else if(username %in% user_base$user){
        #error username already taken
        output$signup_errors <- renderText({
          'Error, username already taken'
        })
      }else if(nchar(password) < 8){
        #error pass word must be 8 characters long
        output$signup_errors <- renderText({
          'Error pass word must be 8 characters long'
        })
      }else if(!grepl('@', email, fixed = T)){
        ...
        showModal(session = session,
                  modalDialog(
                    easyClose = TRUE,
                    size = "l",
                    title = "Please read carefully and accept",
                    footer = actionButton("acceptterms", "Accept terms", width = "160px"),
                    
                    card(
                      style = "border-radius: 10px",
                      layout_columns(
                        col_widths = c(10,-2),
                        p("please accept all terms for using cosmic data for academic purposes only (make nicer later)")
                      )
                    )
                  )
        )
  ...
  observeEvent(input$acceptterms, {
    removeModal()
    query_sql = paste('INSERT INTO user_base VALUES ("', input$new_user, '", "', input$new_password1, '", ', '"standard"', ', "', input$new_email, '")', sep="")
    dbExecute(db_user, query_sql)
    output$signup_errors <- renderText({
      "Redirecting..."
    })
    Sys.sleep(1)
    session$sendCustomMessage(type = 'redirect', quavaprot_host)
  })
