...
#Retrieve canonical GDC mutant info
...
for (n in 1:nrow(df2)){
  print(n)
  if (!(is.na(df2$Mutation_validation_1[n]))){
...
for (n in index){
  print(n)
  suppressWarnings(
    x <- MSIF_pep_generator(consequence = df2$Consequence[n], 
                            protseq = df2$Mutated_GDC_Sequence[n],
                            aachng = df2$AA_Change[n],
                            return = "peptide")
  )
  if (!(is.null(x))){
    df2[n, 'GDC_Mutant_Tryptic_Peptide'] <- x
  }
}
...
for (n in index){
  print(n)
  x <- MSIF_pep_generator(df2$Consequence[n],
                          df2$GDC_Native_Sequence[n],
                          df2$AA_Change[n],
                          return = "All")
  if (!(is.null(x))){
    df2[n, 'GDC_Native_Tryptic_Peptide'] <- x[1]
    df2[n, 'Peptide_start'] <- x[2]
    df2[n, 'Peptide_end'] <- x[3]
  }
}
...
for (n in index){
  print(n)
  if(!(is.na(df2$Mutated_GDC_Sequence[n])) & !(is.na(df2$GDC_Native_Tryptic_Peptide[n]))){
...
dfGDC = dfFinal
dfGDC = dfGDC[,c(1:29,57,30:56,58:66)]
