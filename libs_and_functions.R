...
mutation_processor = function(Table, session){
  ...
  index = grep("missense|stop_gained|inframe_deletion", Table2$Consequence)
  for(n in index){
    Table2$Mutant_sequence[n] = MSI_seq_generator(consequence = Table2$Consequence[n], 
                                                  native_aa = Table2$Native_sequence[n], 
                                                  aachng = Table2$HGVSP[n])
  }
  
  #inframe ins
  index = grep("inframe_insertion", Table2$Consequence)
  for(n in index){
    Table2$Mutant_sequence[n] = inframe_insertion_seq_generator(consequence = Table2$Consequence[n],
                                                                native_aa = Table2$Native_sequence[n],
  ...
  #stop_lost
  index = grep("stop_lost", Table2$Consequence)
  for (n in index){
    print(n)
    id_index = grep(TRUE, (Table2$transcript_id[n] == cdna_list$ensembl_transcript_id))
    Table2$Mutant_sequence[n] = stop_lost_seq_generator(cdna = cdna_list$cdna[id_index], 
                                                        consequence = Table2$Consequence[n], 
                                                        hgvsc = Table2$HGVSC[n])
  }
  
  #frameshift
  index = grep("frameshift", Table2$Consequence)
  for (n in index){
    print(n)
    id_index = grep(TRUE, (Table2$transcript_id[n] == cdna_list$ensembl_transcript_id), fixed = TRUE)
    if (length(id_index) != 0){
      Table2$Mutant_sequence[n] = frameshift_seq_generator(cdna = cdna_list$cdna[id_index],
                                                           native_aa = Table2$Native_sequence[n],
                                                           hgvsc = Table2$HGVSC[n])
    }
  }
  ...
  for(n in index){
    print(n)
    Table2$Mutation_validation_1[n] = MSI_mut_checker(Table2$Consequence[n], Table2$Native_sequence[n], Table2$HGVSP[n])
  }
  
  index = grep("frameshift", Table2$Consequence)
  for(n in index){
    print(n)
    Table2$Mutation_validation_1[n] = frameshift_mut_checker(Table2$Native_sequence[n], Table2$HGVSP[n], Table2$Consequence[n])
  }
  
  index = grep("inframe_insertion", Table2$Consequence)
  for(n in index){
    print(n)
    Table2$Mutation_validation_1[n] = inframe_insertion_mut_checker(Table2$Native_sequence[n], Table2$HGVSP[n], Table2$Consequence[n])
  }
  
  index = grep("stop_lost", Table2$Consequence)
  for(n in index){
    print(n)
    Table2$Mutation_validation_1[n] = stop_lost_mut_checker(Table2$Native_sequence[n], Table2$HGVSP[n], Table2$Consequence[n])
  }
  ...
  for(n in index){
    print(n)
    Table2$Mutation_validation_2[n] = MSI_mut_checker_2(Table2$Consequence[n], Table2$Mutant_sequence[n], Table2$HGVSP[n])
  }
  
  index = grep("frameshift", Table2$Consequence)
  for(n in index){
    print(n)
    Table2$Mutation_validation_2[n] = frameshift_mut_checker_2(Table2$Mutant_sequence[n], Table2$HGVSP[n], Table2$Consequence[n])
  }
  
  index = grep("inframe_insertion", Table2$Consequence)
  for(n in index){
    print(n)
    Table2$Mutation_validation_2[n] = inframe_insertion_mut_checker_2(Table2$Mutant_sequence[n], Table2$HGVSP[n], Table2$Consequence[n])
  }
  
  index = grep("stop_lost", Table2$Consequence)
  for(n in index){
    print(n)
    Table2$Mutation_validation_2[n] = stop_lost_mut_checker_2(Table2$Mutant_sequence[n], Table2$HGVSP[n], Table2$Consequence[n])
  }
  ...
  #mutant peptides
  index = grep(TRUE, Table2$Pass)
  
  for (n in index){
    print(n)
    suppressWarnings(
      x <- MSIF_pep_generator(consequence = Table2$Consequence[n], 
                              protseq = Table2$Mutant_sequence[n],
                              aachng = Table2$HGVSP[n],
                              return = "peptide")
    )
    if (!(is.null(x))){
      Table2[n, 'Mutant_Tryptic_Peptide'] <- x
    }
  }
  ...
  for (n in index){
    print(n)
    x <- MSIF_pep_generator(consequence = Table2$Consequence[n], 
                            protseq = Table2$Native_sequence[n],
                            aachng = Table2$HGVSP[n],
                            return = "All")
    if (!(is.null(x))){
      Table2[n, 'Native_Tryptic_Peptide'] <- x[1]
      Table2[n, 'Peptide_start'] <- x[2]
      Table2[n, 'Peptide_end'] <- x[3]
    }
  }
  
  main_index = grep(TRUE, Table2$Pass)
  ...
  for (n in main_index){
    y = Table2$Native_Canonical_Sequence[n]
  ...
  Table2 = data.frame(Table2, Unique_check=NA)
  
  for (n in main_index){
    y = Table2$Native_Canonical_Sequence[n]
    df3 = trypsin(y, TRUE, TRUE, FALSE)
    Table2$Unique_check[n] <- !(Table2$Mutant_Tryptic_Peptide[n] %in% df3$peptide)
  }
  rm(df3)
  ...
  Table2 <- PTM_Filter(Table = Table2,
                       ...
                       Consequence = Table2$Consequence, 
  ...
  Table2[!Table2$Pass, c("Canonical_check","Unique_check","Mutant_Length_Filter","Native_Length_Filter",
